...
export default function InvoiceShow({ order }: { order: Order }) {
    ...
    const paymentMethod =
        order.payments?.[0]?.payment_method?.toUpperCase() || 'CASH';
    const totalItems =
        order.products?.reduce((sum, p) => sum + (p.quantity || 1), 0) || 0;
                            ...
                            <div>
                                <div className="flex flex-wrap items-center gap-3">
                                    ...
                                    <button
                                        ...
                                        className="rounded-lg bg-blue-50 px-2.5 py-1 text-xs font-bold text-blue-600 transition hover:bg-blue-100"
                                    ...
                                    <span
                                        className={`inline-flex items-center rounded-full border px-3 py-0.5 text-xs font-bold tracking-wider uppercase ${getStatusBadge(order.status)}`}
                                    >
                                        {order.status}
                                    </span>
                                </div>
